...
static struct tx_voltage_settings tx_voltage_array[MAX_TX_VOL_OPT] = {
    ...
    {0x0F, 0x34, 0x11, "Va1.20_Vb0.85_De_m3.00dB"}, /* Va 1.20, Vb 0.85, De-emphasis -3.00dB */
    {0x0F, 0x35, 0x0F, "Va1.20_Vb0.90_De_m2.50dB"}, /* Va 1.20, Vb 0.90, De-emphasis -2.50dB */
    {0x0F, 0x2C, 0x16, "Va1.10_Vb0.67_De_m4.30dB"}, /* Va 1.10, Vb 0.67, De-emphasis -4.30dB */
    {0x0F, 0x2B, 0x18, "Va1.10_Vb0.63_De_m4.84dB"}, /* Va 1.10, Vb 0.63, De-emphasis -4.84dB */
    {0x0F, 0x2A, 0x1A, "Va1.10_Vb0.60_De_m5.26dB"}, /* Va 1.10, Vb 0.60, De-emphasis -5.26dB */
    {0x0F, 0x31, 0x16, "Va1.20_Vb0.75_De_m4.08dB"}, /* Va 1.20, Vb 0.75, De-emphasis -4.08dB */
    {0x0F, 0x30, 0x19, "Va1.20_Vb0.70_De_m4.68dB"}, /* Va 1.20, Vb 0.70, De-emphasis -4.68dB */
    {0x0F, 0x2E, 0x1C, "Va1.20_Vb0.65_De_m5.33dB"}, /* Va 1.20, Vb 0.65, De-emphasis -5.33dB */
};
//static u8 m_TxVoltageIdx = 3; /* Va 1.10, Vb 0.65, De-emphasis -3.5dB (recommended, default) */
static uint m_TxVoltageIdx = 3; /* Va 1.10, Vb 0.65, De-emphasis -3.50dB (recommended, default) */
module_param(m_TxVoltageIdx, uint, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(m_TxVoltageIdx, "Index of Tx volatage setting, 0~16");
...
// bank 1422:0x1f284400
// bank 1433:0x1f286600
// bank 1436:0x1f286c00
// bank 1522:0x1f2a4400
// bank 1523:0x1f2a4600
// bank 1524:0x1f2a4800
// bank 1525:0x1f2a4a00
...
static int phy_infinity6e_sata_init(struct phy *phy)
{
    ...
    miupll_freq = 24 * INREGMSK16(BASE_REG_MIUPLL_PA + REG_ID_03, 0x00FF) /
                ((INREGMSK16(BASE_REG_MIUPLL_PA + REG_ID_03, 0x0700) >> 8) + 2);
    printk("-miupll %d\r\n", miupll_freq);
    if (miupll_freq <= 266) {
        // raise ssusb_axi clock rate to 384MHz
        CLRREG16(BASE_REG_CLKGEN_PA + REG_ID_6F, 0xF000);
        SETREG16(BASE_REG_CLKGEN_PA + REG_ID_6F, 0xC000);
    }

    phy_utmi_deinit(phy);

#if 0
    printk("bank 1422:0x%x\n", BANK_TO_ADDR32(0x1422)+u4IO_PHY_BASE);
    printk("bank 1433:0x%x\n", BANK_TO_ADDR32(0x1433)+u4IO_PHY_BASE);
    printk("bank 1436:0x%x\n", BANK_TO_ADDR32(0x1436)+u4IO_PHY_BASE);
    printk("bank 1522:0x%x\n", BANK_TO_ADDR32(0x1522)+u4IO_PHY_BASE);
    printk("bank 1523:0x%x\n", BANK_TO_ADDR32(0x1523)+u4IO_PHY_BASE);
    printk("bank 1524:0x%x\n", BANK_TO_ADDR32(0x1524)+u4IO_PHY_BASE);
    printk("bank 1525:0x%x\n", BANK_TO_ADDR32(0x1525)+u4IO_PHY_BASE);
#endif
