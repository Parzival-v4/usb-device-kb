...
static struct uac1_output_terminal_descriptor io_out_ot_desc = {
	...
	.wTerminalType		= cpu_to_le16(0x0405),
	.bAssocTerminal		= 0,
	.bSourceID		= 0,
};
...
static struct usb_audio_control_selector feature_unit_mic = {
	...
	//.desc = (struct usb_descriptor_header *)in_feature_unit_desc, //dynamic
};
...
static struct usb_audio_control_selector feature_unit_spk = {
	...
	.name = "Mute & Volume Control",
	.type = UAC_FEATURE_UNIT,
	//.desc = (struct usb_descriptor_header *)out_feature_unit_desc, //dynamic
};

static struct usb_descriptor_header *f_fs_audio_desc[] = {
...
static int f_audio_out_ep_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (as_out_ep_desc.wMaxPacketSize - copy_buf->actual < req->actual) {
	        list_add_tail(&copy_buf->list, &audio->play_queue);
	        schedule_work(&audio->playback_work);
	        copy_buf = f_audio_buffer_alloc(as_out_ep_desc.wMaxPacketSize);
	        if (IS_ERR(copy_buf))
	                return -ENOMEM;
	}
...
static void f_audio_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case 0: 			/* normal completion? */
		if (ep == in_ep){
			...
			//printk(KERN_EMERG "data = 0x%x,uac1 = 0x%p\n", data,uac1);
                ...
                const struct usb_ctrlrequest *ctrl)
{
	...
	if(id == feature_unit_mic.id){
	 	if(con_sel == 0x01){
			uac1->event_type = UAC_MIC_MUTE;
		}else{
			uac1->event_type = UAC_MIC_VOLUME;
		}
	}else if(id == feature_unit_spk.id){
		if(con_sel == 0x01){
			uac1->event_type = UAC_SPEAK_MUTE;
		}else{
			uac1->event_type = UAC_SPEAK_VOLUME;
		}
	}
...
f_audio_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	switch (ctrl->bRequestType) {
		...
		//printk(KERN_EMERG "%s 2 ctrl->bRequestType:%x ctrl->bRequest:%x\n", __func__, ctrl->bRequestType, ctrl->bRequest);
...
static int f_audio_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (intf == uac1->ac_intf) {
		...
		uac_device_stop_capture(&uac1->uac_dev);
	...
	if (intf == uac1->as_out_intf) {
		...
		if (alt) {
			...
			uac_device_start_capture(&uac1->uac_dev);
			//printk(KERN_EMERG "start capture %s\n",__func__);
			memset(&v4l2_event, 0, sizeof(v4l2_event));
			...
			//printk(KERN_EMERG "stop capture %s\n",__func__);
		}
...
static struct uac_feature_unit_descriptor *build_fu_desc(int chmask)
{
    ...
    fu_desc->bmaControls[0] = 0x03;//支持 mute 和 volume，可动态配置是否支持mute和volume时再修改

    return fu_desc;
}

static struct uac1_ac_header_descriptor *build_ac_header_desc(struct f_uac1_opts *opts)
{
 ...
 */
    ...
    if (EPIN_EN(opts))//if (FUIN_EN(opts))
        in_feature_unit_desc->bUnitID = i++;
    if (EPOUT_EN(opts))//if (FUOUT_EN(opts))
    ...
    if (EPIN_EN(opts)) {
        ...
        feature_unit_mic.id = in_feature_unit_desc->bUnitID;
        feature_unit_mic.desc = (struct usb_descriptor_header *)in_feature_unit_desc;
    }
    if (EPOUT_EN(opts)) {
        u16 len = le16_to_cpu(ac_header_desc->wTotalLength);
        ...
        ac_header_desc->wTotalLength = cpu_to_le16(len);
        iad_desc.bInterfaceCount++;
        feature_unit_spk.id = out_feature_unit_desc->bUnitID;
        feature_unit_spk.desc = (struct usb_descriptor_header *)out_feature_unit_desc;
    }

    setup_headers(opts, f_fs_audio_desc, USB_SPEED_FULL);
...
//    }
	...
	if(EPIN_EN(audio_opts) && mic_name[0] != 0) //if (FUIN_EN(audio_opts))
	{
        u8 *i_feature;
        i_feature = (u8 *)in_feature_unit_desc + in_feature_unit_desc->bLength - 1;
        *i_feature = us[STR_MIC_IF].id;
	...
	if(EPOUT_EN(audio_opts) && speak_name[0] != 0) //if (FUOUT_EN(audio_opts))
	{
        u8 *i_feature; 
        i_feature = (u8 *)out_feature_unit_desc + out_feature_unit_desc->bLength - 1;
        *i_feature = us[STR_SPEAK_IF].id;
	...
	fs_as_in_ep_desc.wMaxPacketSize = as_in_ep_desc.wMaxPacketSize;

	//host to divce
	as_out_ep_desc.wMaxPacketSize = num_channels(audio_opts->c_chmask) * audio_opts->c_ssize * (audio_opts->c_srate/1000);
	fs_as_out_ep_desc.wMaxPacketSize = as_out_ep_desc.wMaxPacketSize;

	/* Set sample rates */
	rate = audio_opts->c_srate;
	sam_freq = as_out_type_i_desc.tSamFreq[0];
	memcpy(sam_freq, &rate, 3);
	printk(KERN_EMERG "================sam_freq:%d c_rate:%d\n", *sam_freq, rate);
	rate = audio_opts->p_srate;
	sam_freq = as_in_type_i_desc.tSamFreq[0];
	memcpy(sam_freq, &rate, 3);
	printk(KERN_EMERG "================sam_freq:%d p_rate:%d\n", *sam_freq, rate);
	...
	fs_as_out_ep_desc.bEndpointAddress = as_out_ep_desc.bEndpointAddress;
	...
	ep->driver_data = uac_dev;
	fs_as_in_ep_desc.bEndpointAddress = as_in_ep_desc.bEndpointAddress;
	...
	status = usb_assign_descriptors(f, f_fs_audio_desc, f_audio_desc, f_ss_audio_desc, NULL);
	if (status) {
		printk(KERN_EMERG "%s:%d\n", __func__,__LINE__);
		goto fail;
	}
	...
	printk(KERN_EMERG "xiapengcheng f_audio_bind OK!!!\n");
	return 0;
...
static int control_selector_init(struct f_uac1 *uac1)
{
	...
	list_add(&feature_unit_spk.list, &uac1->cs);
	...
	spk_volume_control.data[UAC__CUR] = speak_volume_def; // -9db
	spk_volume_control.data[UAC__MIN] = speak_volume_min; // -121db
	spk_volume_control.data[UAC__MAX] = speak_volume_max; // -7db
