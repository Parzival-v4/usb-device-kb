...
struct uvc_device *g_uvc_s2 = NULL;
...
#define UVC_STREAMING_INTERFACE_PROTOCOL		0x01//UVC1.5 0x01,UVC1.1 0x00	

static struct usb_string uvc_en_us_strings[] = {
	[UVC_STRING_CONTROL_IDX].s = "UVC Camera 2",
	[UVC_STRING_STREAMING_IDX].s = "Video Streaming 2",
	{  }
};
...
#if (defined CHIP_SSC333 || defined CHIP_SSC337 || defined CHIP_SSC9211 || defined CHIP_CV5) || (defined CHIP_CV72)
...
uvc_function_set_alt(struct usb_function *f, unsigned interface, unsigned alt)
{
	...
	printk("%d %s(%u, %u) uvc->state: %d uvc->control_intf: %d : %d \n",__LINE__,__func__,
		interface, alt, uvc->state, uvc->control_intf, uvc->streaming_intf);
		
		
	if (interface == uvc->control_intf || connect_flag == 0) {
		if (alt)
			return -EINVAL;

		//if (uvc->state == UVC_STATE_DISCONNECTED) 
		{
			printk("stream 2: UVC_EVENT_CONNECT usb speed = %s, bConfigurationValue = %d\n",
...
uvc_copy_descriptors(struct uvc_device *uvc, enum usb_device_speed speed)
{
	...
	*/
	...
	//uvc_control_header->baInterfaceNr[1] = uvc->streaming_intf;
...
uvc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	*/
	...
	uvc->control_intf = 0;
	printk("uvc_s2_enable:0x%02x\n", uvc->uvc_s2_enable);
	if(uvc->uvc_s2_enable & 0x80)
	{
	...
	if(!(uvc->uvc_s2_enable & 0x80))
	{
		if(uvc->streaming_intf >= 2)
			uvc->control_intf = uvc->streaming_intf - 2;
		else
			uvc->control_intf = 0;
	}

		
	if(c->bConfigurationValue == 2)//UVC1.5
	{	
...
static void uvc_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	INFO(cdev, "stream2:%s, bConfigurationValue:%d \n", __func__, c->bConfigurationValue);
...
static struct usb_function *uvc_alloc(struct usb_function_instance *fi)
{
	...
	uvc->zte_hid_enable = 0;//opts->zte_hid_enable;
	uvc->uvc_interrupt_ep_enable = 0;//uvc->zte_hid_enable ? 0 : opts->uvc_interrupt_ep_enable;
...
int __init
uvc_bind_config_s2(struct usb_configuration *c,
	...
	const struct uvc_descriptor_header * const *ss_streaming)
{
	...
	//pr_notice("%s +%d %s: uvc_streaming_intf_alt1.iInterface:%d, then 0\n",
...
void uvc_unbind_config_s2(void){
	if(g_uvc_s2 != NULL ){
		usb_put_function(&g_uvc_s2->func);
		kfree(g_uvc_s2);
	}
}

DECLARE_USB_FUNCTION_INIT(uvc_s2, uvc_alloc_inst, uvc_alloc);
MODULE_LICENSE("GPL");
MODULE_AUTHOR("VHD");
