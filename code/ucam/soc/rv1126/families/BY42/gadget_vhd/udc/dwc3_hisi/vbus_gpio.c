...
static unsigned long gpio_base = 0xff640000;
module_param(gpio_base, ulong, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(gpio_base, "gpio_reg_base");

static unsigned long gpio_data_offset = DATA_OFFSET;
module_param(gpio_data_offset, ulong, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(gpio_data_offset, "gpio_data_offset");

static unsigned long gpio_dir_offset = DIR_OFFSET;
module_param(gpio_dir_offset, ulong, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(gpio_dir_offset, "gpio_dir_offset");

static unsigned long gpio_data = 0x40;
...
static int  vbus_reg_init(void)
{
    ...
    reg_gpio_base = (void*)ioremap(gpio_base, 0x1000);
...
static int vbus_thread(void *__unused)
{
	...
	diable_disconn_irq();
    if(gpio_dir_offset){
        t = SYS_READ((unsigned long)reg_gpio_base+gpio_dir_offset);
        SYS_WRITEL((unsigned long)reg_gpio_base+gpio_dir_offset, t&(~gpio_data));
    }
	...
	while(!kthread_should_stop())
	{
		...
		data = SYS_READ((unsigned long)reg_gpio_base+gpio_data_offset) & gpio_data;
		if(data2 != data  && get_usb_soft_connect_status())
		{			
			msleep(20);
			if(data == (SYS_READ((unsigned long)reg_gpio_base+gpio_data_offset) & gpio_data))
			{
				if(data)
				{
					printk("usb connect\n");
...
#if (defined CHIP_VS816 || defined CHIP_VS819)
int vbus_gpio_for_canary_test(char *test1)
{
    char test2[64];
