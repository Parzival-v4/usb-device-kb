...
//lxh_20231101
static int tcpm_set_prepolarity(struct tcpm_port *port,
			     enum typec_cc_polarity polarity)
{
	int ret;

	printk("polarity %d", polarity);

	ret = port->tcpc->set_prepolarity(port->tcpc, polarity);
	if (ret < 0)
		return ret;
...
static void tcpm_mdelay(uint32_t msecs)
{
	if (in_interrupt())
		mdelay(msecs);
	else
			    ...
			    enum typec_cc_status cc2)
{
    ...
    //lxh_20231101
    ret = tcpm_set_prepolarity(port, port->cc2 != TYPEC_CC_OPEN ?
				TYPEC_POLARITY_CC2 : TYPEC_POLARITY_CC1);
	if (ret < 0)
		return ;
