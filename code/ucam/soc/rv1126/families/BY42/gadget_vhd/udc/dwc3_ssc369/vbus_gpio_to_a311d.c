...
static unsigned gpio_num2 = 0xFFFF;
module_param(gpio_num2, uint, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(gpio_num2, "vbus_gpio_num2");
...
{
	int data,data2;
	int gpio2_data,gpio2_data2;
	int vbus_gpio_check = 0;
	...
	while(!kthread_should_stop())
	{
		...
		if(gpio_num2 != 0xFFFF)
		{
			gpio2_data = __gpio_get_value(gpio_num2);
			if(gpio2_data2 != gpio2_data && get_usb_soft_connect_status())
			{			
				msleep(20);
				if(gpio2_data == __gpio_get_value(gpio_num2))
				{	
					if(gpio2_data == 1)//to pc
					{
						printk("usb switch to pc\n");
						vbus_gpio_check = 0;	
					...
					else if(gpio2_data == 0)//to A311D
					{
						printk("usb switch to A311D\n");
						vbus_gpio_check = 0;
						vbus_usb_soft_disconnect();
						dwc3_gadget_disconnect_vbus();
						msleep(delay_ms);
...
int vbus_gpio_init(void) 
{
	...
	if(gpio_num2 != 0xFFFF)
	{
		ret = gpio_request(gpio_num2, "vbus_gpio");
		if(ret != 0)
		{
			printk("gpio_request error! ret:%d\n",ret);
			return ret;
		...
		ret = gpio_direction_input(gpio_num2);
		if(ret != 0)
		{
			printk("gpio_direction_input error! ret:%d\n",ret);
			return ret;
