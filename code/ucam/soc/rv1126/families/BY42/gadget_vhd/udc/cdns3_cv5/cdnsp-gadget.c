...
void cdnsp_clear_port_change_bit(struct cdnsp_device *pdev,
					__le32 __iomem *port_regs)
{
...
int cdnsp_ep_enqueue(struct cdnsp_ep *pep, struct cdnsp_request *preq)
{
    ...
    if (preq->epnum == 0)
    {
        preq->direction = pdev->ep0_expect_in;
    }
...
void cdnsp_stop(struct cdnsp_device *pdev)
{
	u32 temp;
...
static struct usb_gadget *g_gadget = NULL;
...
int cdnsp_port_connect(int is_on)
{
	struct cdnsp_device *pdev;
	struct cdns *cdns;
	unsigned long flags;
	...
	pdev = gadget_to_cdnsp(g_gadget);
	cdns = dev_get_drvdata(pdev->dev);
    
    if(!pdev || !pdev->port_connect)
    {
        return 0;
    }

    //printk("cdnsp_port_connect is_on:%d\n",is_on);
...
EXPORT_SYMBOL_GPL(cdnsp_port_connect);

static void cdnsp_gadget_get_config_params(
	struct usb_gadget *g,
...
static const struct usb_gadget_ops cdnsp_gadget_ops = {
	...
	.get_config_params = cdnsp_gadget_get_config_params,
};
...
void cdnsp_irq_reset(struct cdnsp_device *pdev)
{
    ...
    if (pdev->gadget.speed != USB_SPEED_UNKNOWN)
    {
        spin_unlock(&pdev->lock);
	...
	printk("usb speed:%s\n", usb_speed_string(pdev->gadget.speed));
...
int cdnsp_gadget_suspend(struct cdns *cdns, bool do_wakeup)
{
	struct cdnsp_device *pdev = cdns->gadget_dev;
	unsigned long flags;
...
EXPORT_SYMBOL_GPL(cdnsp_gadget_suspend);

int cdnsp_gadget_resume(struct cdns *cdns, bool hibernated)
{
	struct cdnsp_device *pdev = cdns->gadget_dev;
...
EXPORT_SYMBOL_GPL(cdnsp_gadget_resume);
