                     ...
                     gfp_t gfp_flags)
{
    ...
    if (unlikely(!_req || !_req->complete || !list_empty(&req->queue)))
    {
        if (!_req)
            printk("%s: 1 X X X\n", __FUNCTION__);
        else
                       ...
                       struct usb_request *_req)
{
    ...
    {
#if 0
        printk(KERN_DEBUG"dequeued req %p from %s, len %d\n",
            req, _ep->name, _req->length);
#endif
