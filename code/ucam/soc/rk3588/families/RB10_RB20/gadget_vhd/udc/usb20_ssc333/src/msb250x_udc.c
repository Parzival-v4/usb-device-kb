...
static irqreturn_t msb250x_udc_ep_isr(struct msb250x_udc *dev)
{
    ...
    for (ep_num = MSB250X_MAX_ENDPOINTS - 1; ep_num > 0; ep_num--)
    {
...
static irqreturn_t msb250x_udc_dma_isr(struct msb250x_udc *dev)
{
    ...
    if (dma_intr)
    {
        ...
        for (dma_ch = MSB250X_USB_DMA_CHANNEL -1 ; dma_ch >= 0; dma_ch --)
        {
...
#if defined(CONFIG_ARCH_PIONEER3)
...
void msb250x_udc_init_otg(struct msb250x_udc *udc)
{
    ...
    */
    ...
    //ms_writeb(usb_intr, MSB250X_OTG0_INTRUSBE_REG);
