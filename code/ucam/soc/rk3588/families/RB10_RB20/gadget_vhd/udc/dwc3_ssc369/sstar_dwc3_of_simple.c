...
__maybe_unused static void dwc3_of_simple_setup_timer(struct dwc3_of_simple *simple)
{
...
static int dwc3_of_simple_sstar_init_mode(struct platform_device *pdev)
{
    ...
    do
    {
        ...
        // Select USB30 SATA_TX250M_CK to debug bus test clock
        regmap_clear_bits(drd_ctrl, (0x20 << 2), BIT(1) | BIT(0));
        ret = regmap_write(drd_ctrl, (0x20 << 2), 0x0002); // Select USB30 PHY
        if (ret)
        {
            dev_err(dev, "Select USB30 PHY fail(%d)\n", ret);
            break;
        }
...
#ifndef CONFIG_TEST_ALL_LINUX_MMU_MAP_ADDRESS
#define CONFIG_TEST_ALL_LINUX_MMU_MAP_ADDRESS 0
#endif
        // dma_direct_set_offset(dev, CONFIG_MIU0_BUS_BASE, CONFIG_TEST_ALL_LINUX_MMU_MAP_ADDRESS, lx_mem_size);

        ret = dwc3_of_simple_sstar_init_mode(pdev);
...
static const struct of_device_id of_dwc3_simple_match[] = {
    {.compatible = "sstar,infinity7-dwc3"}, {.compatible = "sstar,mercury6p-dwc3"}, {/* Sentinel */}};
