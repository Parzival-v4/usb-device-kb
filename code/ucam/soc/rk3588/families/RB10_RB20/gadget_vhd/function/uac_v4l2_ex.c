...
uac_v4l2_querycap(struct file *file, void *fh, struct v4l2_capability *cap)
{
	...
	cap->capabilities = cap->device_caps | V4L2_CAP_DEVICE_CAPS;
#else
	cap->capabilities = V4L2_CAP_AUDIO | V4L2_CAP_STREAMING;
#endif
...
#define UVCIOC_SEND_RESPONSE            		_IOW('U', 1, struct uac_request_data)
		...
		unsigned int cmd, void *arg)
{
    ...
    //printk(KERN_INFO "%s:%d cmd:%x UVCIOC_SEND_RESPONSE:%x \n", __func__, __LINE__, cmd, UVCIOC_SEND_RESPONSE);

	switch (cmd) {
	    case UVCIOC_SEND_RESPONSE:
	    //case 1:
		//return uvc_send_response(uvc, arg);
		return uac_send_response(uac, arg);
