...
rndis_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	//ep = usb_ep_autoconfig_ss_backwards(cdev->gadget, &fs_in_desc, NULL);
	ep = usb_ep_autoconfig_ss(cdev->gadget, &fs_in_desc, NULL);//EP必须是0x81 ?
	if (!ep)
		goto fail;
	rndis->port.in_ep = ep;
	...
	ep = usb_ep_autoconfig_ss_backwards(cdev->gadget, &fs_notify_desc, NULL);
	//ep = usb_ep_autoconfig_ss(cdev->gadget, &fs_notify_desc, NULL);
	if (!ep)
		goto fail;
	rndis->notify = ep;

	ep = usb_ep_autoconfig_ss_backwards(cdev->gadget, &fs_out_desc, NULL);
	//ep = usb_ep_autoconfig_ss(cdev->gadget, &fs_out_desc, NULL);
	if (!ep)
		goto fail;
