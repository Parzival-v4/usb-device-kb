...
static void trim_ic_verify(void)
{
    ...
    val = INREG16(GET_REG_ADDR(0x1F203E00, 0x54));
    tmp = (val >> 10) & 0x003F; //read bit[15:10]
    trim = (INREG16(0x1F2A4740) >> 10) & 0x003F; //1523_50 bit[15:10]
    if (tmp != trim) {
        goto not_trim;
    }

    tmp = (val >> 5) & 0x001F; //read bit[9:5]
	trim = (INREG16(0x1F2A46F0) >> 4) & 0x001F; // 1523_3c bit[8:4]
    if (tmp != trim) {
        goto not_trim;
    }

    tmp = val & 0x001F; //read bit[4:0]
	trim = (INREG16(0x1F2A46A8) >> 7) & 0x001F; // 1523_2a bit[11:7]
    if (tmp != trim) {
        goto not_trim;
    }
...
void sstar_usb_phy_setting(struct phy *phy, uint vcm_cur, uint drv_cur, uint dem_cur)
{
    ...
    printk("-Tx cur vcm[x%X], drv[x%X], dem[x%X]\n", vcm_cur, drv_cur, dem_cur);
    ...
    SETREG16(dphy_base + 0x26*4, (vcm_cur & 0x7) << 1);
    ...
    OUTREG16(dphy_base + 0x40*4, ((dem_cur & 0x0F) << 12) |
                                 ((drv_cur & 0x3F) << 6) | 0x19);
    OUTREG16(dphy_base + 0x41*4, ((dem_cur & 0x30) >> 4) | 0x2188);
    ...
    ms_writeb((ms_readb(GET_REG_ADDR(UTMI_BASE_ADDR, 0x1E)) | 0x01), GET_REG_ADDR(UTMI_BASE_ADDR, 0x1E)); // set CA_START as 1
    mdelay(10);
    ...
    ms_writeb((ms_readb(GET_REG_ADDR(UTMI_BASE_ADDR, 0x02)) | 0x80), GET_REG_ADDR(UTMI_BASE_ADDR, 0x02)); //avoid glitch
