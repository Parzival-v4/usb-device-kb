...
#define MISC_CTRL1		0x4
#define USB2_PHY_CTRL_U3	BIT(15)
...
static void hisi_usb3_phy_eye(struct phy *phy)
{
	...
	reg = readl(priv->misc_ctrl + 0x20);//SYS AXI 总线端口 M3(USB3_PCIE) 优先级
        reg |= 0x700;//把USB设置成最高优先级
        writel(reg, priv->misc_ctrl + 0x20);
    
        reg = readl(priv->misc_ctrl + 0x60);//DDR QOS 控制寄存器 USB3.0_PCIe 写命令在 MDDRC 中的 QOS 值
        reg &= 0xf0ffffff;//把USB设置成最高优先级
        //reg |= 0x01000000; 
        writel(reg, priv->misc_ctrl + 0x60);
    
        reg = readl(priv->misc_ctrl + 0x70);//DDR QOS 控制寄存器 USB3.0_PCIe 读命令在 MDDRC 中的 QOS 值
        reg &= 0xf0ffffff;//把USB设置成最高优先级
        //reg |= 0x01000000; 
        writel(reg, priv->misc_ctrl + 0x70);
#endif

#if defined(CONFIG_ARCH_HI3519) || defined(CONFIG_ARCH_HI3519V101) || defined(CONFIG_ARCH_HI3516AV200) || defined (CONFIG_ARCH_HI3559) || defined (CONFIG_ARCH_HI3556)
	/*
	【问题需求】设置U3的U2 phy来源和时钟来源
	【原因分析】对Hi3519V101/Hi3516AV200系列芯片，只有一个
	 USB2.0的phy，所以在使能USB3.0的时候，只能配置XHCI使用这个phy，不能使
	 能EHCI,否则XHCI无法兼容2.0设备，同时在识别某些3.0设备的时候，由于无法
	 切换到2.0模式，可能存在兼容性问题（某些3.0设备需要先切2.0识别，再切换
	 到3.0）
	 */
	/*
	 * misc_ctrl1   bit 15(USB2.0 phy control source)
	 * 0: usb2.0 phy controlled by U2.
	 * 1: usb2.0 phy controlled by U3.
	 *
	 * set u2 phy controller to usb3.0 controller
	 */
	reg = readl_relaxed(priv->misc_ctrl + MISC_CTRL1);
	reg |= USB2_PHY_CTRL_U3;
	writel_relaxed(reg, priv->misc_ctrl + MISC_CTRL1);
#endif
}
...
static int hisi_usb3_ctrl_phy_config(struct phy *phy)
{
	...
	reg &= ~(0x3<<12);
	reg |= (0x2<<12); /*[13:12] 01: Host; 10: Device; 11: OTG*/
	reg &= ~U2RSTECN;
...
static int hisi_usb3_phy_on(struct phy *phy)
{
	...
	mdelay(10);
#else
	reg = readl_relaxed(priv->peri_ctrl + 0xb4);
	writel_relaxed(0xf9, priv->peri_ctrl + 0xb4);
	mdelay(10);

	writel_relaxed(0x1f01, priv->peri_ctrl + USB3_CTRL);
	//writel_relaxed(0x3f01, priv->peri_ctrl + USB3_CTRL);
	mdelay(10);
