...
#if (defined CHIP_CV5)
extern const struct vb2_mem_ops vb2_vmalloc_memops_cv5;
#elif (defined CHIP_RK3588)
extern const struct vb2_mem_ops vb2_vmalloc_memops_rk3588;
#endif
...
static int uvc_buffer_prepare(struct vb2_buffer *vb)
{
	...
	printk("video->reqbufs_config.enable=%d", video->reqbufs_config.enable);
	if(video->cma_dma_buf_enable)
	{
		if(video->reqbufs_config.enable)
			buf->mem = video->reqbufs_addr.ppVirtualAddress[buf->buf.vb2_buf.index];
		else
	...
	printk("uvc_buffer_prepare: idx=%u mem=%p len=%u bytesused=%u cma=%u req_en=%u count=%u\n",
		buf->buf.vb2_buf.index, buf->mem, buf->length, buf->bytesused,
		video->cma_dma_buf_enable, video->reqbufs_config.enable,
		video->reqbufs_addr.count);
...
#if (defined CHIP_CV5)
	queue->queue.mem_ops = &vb2_vmalloc_memops_cv5;
#elif (defined CHIP_RK3588)
	queue->queue.mem_ops = &vb2_vmalloc_memops_rk3588;
#else
