		...
		struct uvc_buffer *buf)
{
	...
	else
	{
...
#endif
		/* Fallback to copy mode for bulk streaming on 6.1 (avoid DMA map errors). */
		req->buf = ureq->req_buffer;
		mem = req->buf;
		...
		/* Process video data (copy from DDR). */
		len = min((int)(video->max_payload_size - video->payload_size), len);
		ret = uvc_video_encode_data_ddr_to_usb(video, buf, mem, len);
...
#endif
		/* Fallback to copy mode for isoc streaming on 6.1 (avoid DMA map errors). */
		req->buf = ureq->req_buffer;
		mem = req->buf;
...
tran_zero:
		...
		{
			struct uvc_request *ureq = req->context;
			if ((req->buf == NULL && req->dma == 0) || (req->length > 0 && req->buf == NULL && ((req->dma & 0xf) != 0))) {
				printk(KERN_ERR "uvc_pump: risky request! buf=%p dma=%llx len=%u idx=%u\n",
					req->buf, (unsigned long long)req->dma, req->length, ureq->vb2_buf_index);
			}
		}
