...
//lxh_20231101
static int tcpci_set_prepolarity(struct tcpc_dev *tcpc,
			      enum typec_cc_polarity polarity)
{
	int ret;
	struct tcpci *tcpci = tcpc_to_tcpci(tcpc);
	if (tcpci->data->set_prepolarity) {
		ret = tcpci->data->set_prepolarity(tcpci, tcpci->data, polarity);
		/* Bypass when ret > 0 */
		if (ret != 0)
			return ret < 0 ? ret : 0;
	}
...
new_alert:
	...
	printk("tcpci_irq: status = 0x%x", status);
	/*
    ...
    //Make sure TCPC_ALERT is cleared completely
	tcpci_read16(tcpci, TCPC_ALERT, &status);
	if (status != 0) {
		printk("Tcpci_irq: new alert come into being");
		goto new_alert;
	}
	return IRQ_HANDLED;
}
...
struct tcpci *tcpci_register_port(struct device *dev, struct tcpci_data *data)
{
    ...
    //lxh_20231101
    tcpci->tcpc.set_prepolarity = tcpci_set_prepolarity;
