...
static int usb_max_speed = 0;
module_param(usb_max_speed, int, 0644);
MODULE_PARM_DESC(usb_max_speed, "usb_max_speed.");
...
static int cdns3_plat_probe(struct platform_device *pdev)
{
	...
	if(usb_max_speed <= 0)
	{
		cdns->usb_max_speed = usb_get_maximum_speed(cdns->dev);
	}
	else
	{
		cdns->usb_max_speed = usb_max_speed;
	}

	if(cdns->usb_max_speed <= USB_SPEED_UNKNOWN)
	{
		cdns->usb_max_speed = USB_SPEED_SUPER_PLUS;
	}
