...
static void dwc3_outstanding_pipe_choose(struct dwc3 *dwc)
{
	...
	//reg |= DWC3_GSBUSCFG0_INCR16BRSTENA | DWC3_GSBUSCFG0_INCR8BRSTENA;
...
static void dwc3_core_num_eps(struct dwc3 *dwc)
{
	...
	dwc->num_in_eps = 6;//DWC3_NUM_IN_EPS(parms);
	dwc->num_out_eps = 6;//DWC3_NUM_EPS(parms) - dwc->num_in_eps;
#else
...
//#if (defined CHIP_VS816)
    if(dwc->num_in_eps > 8)
        dwc->num_in_eps = 8;
    if(dwc->num_out_eps > 8)
        dwc->num_out_eps = 8;
//#endif
...
static int dwc3_probe(struct platform_device *pdev)
{
	...
	dwc3_regs_soft_connect_init(regs);
    ...
    if (usb3_enable == 0 && dwc->dr_mode == USB_DR_MODE_PERIPHERAL)
    {
        dwc->maximum_speed = USB_SPEED_HIGH;
    }
...
|| defined (CONFIG_ARCH_HI3518EV300) || defined (CONFIG_ARCH_HI3516DV200)
	...
	dwc->eps_new_init = device_property_read_bool(dev,"snps,eps_new_init");
...
int dwc3_runtime_suspend(struct device *dev)
{
...
int dwc3_runtime_resume(struct device *dev)
{
	struct dwc3     *dwc = dev_get_drvdata(dev);
	int		ret;
...
int dwc3_runtime_idle(struct device *dev)
{
	struct dwc3     *dwc = dev_get_drvdata(dev);
