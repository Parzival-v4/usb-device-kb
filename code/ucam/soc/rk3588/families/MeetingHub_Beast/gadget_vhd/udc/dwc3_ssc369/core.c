...
static void dwc3_enable_clk(struct device *dev, int enable)
{
	...
	if (num_parents > 0)
	{
		clks = kzalloc((sizeof(struct clk *) * num_parents), GFP_KERNEL);
		if (clks == NULL)
		{
			dev_err(dev, "kzalloc for clk failed\n");
			return;
		}
...
static int dwc3_probe(struct platform_device *pdev)
{
	...
	struct resource		*res;
	struct dwc3		*dwc;
#if (!defined CHIP_SSC268) && (!defined CHIP_SSC369)
	struct clk* clock;
#endif
...
#ifdef CHIP_SSC268
	dwc3_enable_clk(&pdev->dev, false);
#endif
